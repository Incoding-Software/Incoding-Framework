...
function IncMustacheTemplate(data, template) {
    ...
    this.prepare = function(source) {
        ...
        $(['Sum', 'Max', 'Min', 'First', 'Last', 'Average', 'Count', 'Index']).each(function() {

            if (this == 'Index' && source.contains('{{IncTemplateIndex}}')) {
                var i = 0;
                $(_.isArray(currentData) ? currentData : [currentData]).each(function() {
                    this.IncTemplateIndex = i;
                    i++;
                });
                return true;
            }
