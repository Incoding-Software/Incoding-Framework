...
function AjaxAdapter() {
    ...
    this.params = function(data) {
        ...
        $(data).each(function() {
            ...
            if (!ExecutableHelper.IsNullOrEmpty(value) && (_.isArray(value) || value.toString().contains(','))) {
    ...
    this.request = function(options, callback) {
        ...
        $.extend(options, {
            ...
            success : function(data) {
                ...
                $(document).trigger(IncSpecialBinds.IncAjaxSuccess);
            },
            beforeSend : function(jqXHR, settings) {
                $(document).trigger(IncSpecialBinds.IncAjaxBefore);
            },
            complete : function(jqXHR, textStatus) {
                $(document).trigger(IncSpecialBinds.IncAjaxComplete);
            },
            error : function(jqXHR, textStatus, errorThrown) {
                $(document).trigger(IncSpecialBinds.IncAjaxError);
            },
...
function ExecutableHelper() {
    ...
    this.TrySetValue = function(element, val) {
        ...
        if ($(element).is(':checkbox')) {
            ...
            $(onlyCheckBoxes).removeProp('checked');
            if ($(onlyCheckBoxes).length == 1) {
                if (ExecutableHelper.ToBool(val)) {
                    $(onlyCheckBoxes).prop('checked', 'checked');
                }
            } else {
        ...
        if ($(element).is('select') && $(element).length > 1) {
            ...
            $(arrayVal).each(function() {
                $('option[value="{0}"]'.f(this)).closest('select').val(this);
            });
            return;
        }

        if ($(element).is(':radio')) {
            $('input[name={0}][value="{1}"]'.f($(element).prop('name'), val)).prop('checked', 'checked');
            return;
        }
...
function IncMustacheTemplate(data, template) {
    ...
    var prepare = function(source) {
        ...
        var replaceByType = function(type) {
            var pattern = "{{#IncTemplate" + type + "}}(.*?){{/IncTemplate" + type + "}}";
            ...
            if (_.isArray(data)) {
                ...
                var parseData = data.select(function(item) {
                    // ReSharper restore UnusedLocals
                    if (type === 'Count') {
                        return item;
                    }

                    if (type === 'Max' || type === 'Min' || type === 'Average' || type === 'Sum') {
                        return parseFloat(item[property]);
                    } else {
                        return item[property];
                    }
                });
                value = eval('parseData.{0}()'.f(type.toString().toLowerCase()));
            } else {
                value = data[property];
            }

            return source.replaceAll("{{#IncTemplate" + type + "}}" + property + "{{/IncTemplate" + type + "}}", value);
        };

        var specialType = ['Sum', 'Max', 'Min', 'First', 'Last', 'Average', 'Count'];

        $(specialType).each(function() {
            source = replaceByType(this);
        });
    ...
    this.render = function() {
        return Mustache.to_html(prepare(this.template), { data : this.data });
    };
...
function IncodingMetaElement(element) {
    ...
    this.bind = function(eventName, status) {
        ...
        $.each(IncSpecialBinds.DocumentBinds, function() {
            ...
            $(document).bind(this, function(e, result) {
        ...
        $(this.element).bind(eventName, function(e, result) {
...
function IncodingResult(result) {
    ...
    this.isRedirectTo = function() {
        return !ExecutableHelper.IsNullOrEmpty(this.redirectTo);
    };
    ...
    if ($.history) {
        $.history.init(function() {
            if (initializedHistoryPlugin) {
                jQuery.event.trigger(jQuery.Event(IncSpecialBinds.IncChangeUrl));
            }
...
ExecutableBase.prototype = {
    ...
    tryGetVal : function(variable) {
        ...
        if (_.isString(variable)) {
            var isJqueryVariable = variable.toString().startsWith("$(") && variable.toString().endWith(")");
            element = isJqueryVariable ? eval(variable) : variable;
        }
        return ExecutableHelper.Instance.TryGetVal(element);
    }
};
...
ExecutableInsert.prototype.internalExecute = function(data) {
    ...
    if (!ExecutableHelper.IsNullOrEmpty(current.jsonData.template)) {
        ...
        if (current.jsonData.template.contains('/')) {
            $.ajax({
                url : current.jsonData.template,
                async : false,
                cache : true,
                data : {},
                success : function(template) {
                    insertContent = TemplateFactory.Create('Mustache', insertContent, template).render();
                }
            });
        } else {
    ...
    $(document).trigger(IncSpecialBinds.IncInsert);
};
...
ExecutableStoreFetch.prototype.internalExecute = function() {
    ...
    $.eachProperties(fparam, function() {
        ...
        var element = $(current.target).is(selector) ? current.target : $(selector, current.target);

        ExecutableHelper.Instance.TrySetValue(element, fparam[this]);
        return true;
    });
...
//#region class ConditionalUrl extend from ConditionalBase

incodingExtend(ConditionalUrl, ConditionalBase);

function ConditionalUrl() {
}

ConditionalUrl.prototype.isInternalSatisfied = function() {
